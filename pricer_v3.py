...
with tab_swap:
    ...
    with colA:
        ...
        p = CCY_PRESETS[ccy]
        side = st.selectbox("Swap side", ["Payer fixed", "Receiver fixed"],
                            index=["Payer fixed","Receiver fixed"].index(st.session_state.swap["side"]))
        notional = st.number_input(f"Notional ({ccy})", 1e5, 1e12,
                                   float(st.session_state.swap["notional"]), step=1e5, format="%.0f")
        maturity = st.number_input("Maturity (years)", 0.5, 60.0,
                                   float(st.session_state.swap["maturity"]), step=0.5)
        fixed_freq = st.selectbox("Fixed payments per year", [1,2,4],
                                  index=[1,2,4].index(st.session_state.swap["fixed_freq"]))
        ...
        manual_K = st.number_input("Manual fixed rate K (decimal)", 0.0, 1.0,
                                   float(st.session_state.swap["manual_K"]), step=0.0005, format="%.4f")
    ...
    if do_price:
        ...
        st.session_state.swap = {
            "ccy": ccy, "side": side, "notional": notional, "maturity": maturity,
...
with tab_cva:
    ...
    notional = float(sw["notional"])
    side = sw["side"]
    K_used = float(sw["par_rate"]) if np.isfinite(sw["par_rate"]) else float(sw["manual_K"])
    zero_times, zero_rates = sw["zero_times"], sw["zero_rates"]

    # Show the inherited swap setup
    st.info(f"Using swap from Tab 1 — CCY: **{ccy}**, Side: **{side}**, Notional: **{notional:,.0f}**, "
    ...
    with colL:
        ...
        if credit_mode == "Constant hazard λ":
            ...
            st.caption("CDS bootstrap omitted; a flat proxy will be used.")
...
with tab_bar:
    ...
    with c1:
        kappa_h = st.number_input("κ (mean reversion)", 0.0001, 10.0, 1.5, step=0.05, format="%.4f")
    with c2:
        theta_h = st.number_input("θ (long-run var)", 0.0001, 5.0, 0.04, step=0.001, format="%.4f")
    with c3:
        xi_h = st.number_input("ξ (vol of vol)", 0.0001, 5.0, 0.5, step=0.01, format="%.4f")
    with c4:
        rho_h = st.number_input("ρ (corr S–v)", -0.9999, 0.9999, -0.7, step=0.01, format="%.4f")
    with c5:
        v0_h = st.number_input("v₀ (initial var)", 0.0001, 5.0, 0.04, step=0.001, format="%.4f")
    ...
    if go_bar:
        ...
        t_grid, S_paths, v_paths = heston_paths_euler(
            S0, v0_h, T_opt, r_eff, q_div, kappa_h, theta_h, xi_h, rho_h,
        ...
        # Payoff vs S(T) — compact chart (smaller)
        st.markdown("**Payoff vs terminal price S(T)**")
        st.write("Dots: simulated payoffs; line: vanilla payoff; vertical line: barrier.")
        ...
        with left:
            ...
            fig, ax = plt.subplots(figsize=(4, 2.6), dpi=120)  # smaller figure
            ax.scatter(ST, payoff_paths, s=5, alpha=0.35)
            ax.plot(ST_axis, vanilla_curve, linewidth=1.8)
